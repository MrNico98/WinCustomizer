...
<Window x:Class="wincustomizergui.MainWindow"
    ...
    <Grid Background="#FF96FF83">
        ...
        <Label Content="Ver.: 4.0" HorizontalAlignment="Left" Height="30" Margin="368,14,0,0" VerticalAlignment="Top" Width="62" Grid.Row="1"/>
        <Label Content="Developer: MrNico98" HorizontalAlignment="Left" Height="30" Margin="0,14,0,0" VerticalAlignment="Top" Width="133" Grid.Row="1"/>
        <Button Content="App Debloat" x:Name="deb" Margin="145,357,125,0" Background="White" BorderBrush="White" Foreground="#FFFF0909" Focusable="False" ScrollViewer.CanContentScroll="False" AllowDrop="True" Height="35" VerticalAlignment="Top" Grid.RowSpan="2"/>
        <Image x:Name="debloimage" Source="https://raw.githubusercontent.com/MrNico98/Icone-Wincustomizer/main/Picsart_23-09-22_22-29-23-192.png" RenderTransformOrigin="0.5,0.5" Width="30" Height="30" ScrollViewer.CanContentScroll="True" AllowDrop="True" VerticalAlignment="Top" HorizontalAlignment="Left" Margin="148,357,0,0" Grid.RowSpan="2"/>
        <TextBlock HorizontalAlignment="Left" Height="30" Margin="106,6,0,0" TextWrapping="Wrap" VerticalAlignment="Top" Width="238" FontFamily="Jokerman" FontSize="22" FontWeight="Bold"><Run Language="it-it" Text="WinCustomizerGUI"/></TextBlock>
    </Grid>
</Window>
"@
...
$WPFtooliso.Add_Click({
    ...
    if (-not (Test-Path -Path "$currentPath\isotool11.exe" -PathType Leaf)) {
    ...
    if (-not (Test-Path -Path "$currentPath\isotool10exe" -PathType Leaf)) {
    Error('Il programma non presente')} else {True('FATTO')}
    })
...
$WPFdeb.Add_Click({
    ...
    if (Test-Path -Path "$env:USERPROFILE\Desktop"-PathType Leaf) {copy "Risorse\MrNico98Debloat3.0.exe" "$env:USERPROFILE\OneDrive\Desktop"}
    warning('App copiata sul desktop') 
    if (-not (Test-Path -Path "$currentPath\Risorse\MrNico98Debloat3.0.exe" -PathType Leaf)) {
...
cmd.exe /c reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\Search" /v "BingSearchEnabled" /t "REG_DWORD" /d 0 /f
cmd.exe /c reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\Themes\Personalize" /v "AppsUseLightTheme" /t "REG_DWORD" /d 0 /f
cmd.exe /c reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\Themes\Personalize" /v "SystemUsesLightTheme" /t "REG_DWORD" /d 0 /f
cmd.exe /c reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\Themes\Personalize" /v "EnableTransparency" /t "REG_DWORD" /d 1 /f
cmd.exe /c reg add "HKCU\Control Panel\Colors" /v "ImmersiveApplicationForeground" /t REG_SZ /d "255 255 255" /f
cmd.exe /c reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\Explorer" /v "AllowOnlineTips" /t "REG_DWORD" /d 0 /f
cmd.exe /c del /f /s /q /a "%AppData%\Microsoft\Internet Explorer\Quick Launch\User Pinned\TaskBar\*"
cmd.exe /c reg delete "HKCU\Software\Microsoft\Windows\CurrentVersion\Explorer\Taskband" /f
cmd.exe /c reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\Search" /v "SearchboxTaskbarMode" /t "REG_DWORD" /d 0 /f
cmd.exe /c reg add "HKLM\SOFTWARE\Policies\Microsoft\Windows\Windows Chat" /v ChatIcon /t "REG_DWORD" /d 3 /f
cmd.exe /c reg add "HKCU\Control Panel\UnsupportedHardwareNotificationCache" /v "SV1" /t "REG_DWORD" /d 0 /f
cmd.exe /c reg add "HKCU\Control Panel\UnsupportedHardwareNotificationCache" /v "SV2" /t "REG_DWORD" /d 0 /f
cmd.exe /c reg add "HKLM\SYSTEM\Setup\MoSetup" /v "AllowUpgradesWithUnsupportedTPMOrCPU" /t "REG_DWORD" /d 1 /f
cmd.exe /c reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\ContentDeliveryManager" /v "OemPreInstalledAppsEnabled" /t "REG_DWORD" /d 0 /f
cmd.exe /c reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\ContentDeliveryManager" /v "PreInstalledAppsEnabled" /t "REG_DWORD" /d 0 /f
cmd.exe /c reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\ContentDeliveryManager" /v "SilentInstalledAppsEnabled" /t "REG_DWORD" /d 0 /f
...
cmd.exe /c sc config wsearch start=auto
cmd.exe /c reg add "HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced" /v TaskbarDa /t "REG_DWORD" /d 0 /f
True('FATTO')
 }

if (10 -eq $os) {
cmd.exe /c reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer" /v "HideSCAMeetNow" /t "REG_DWORD" /d 1 /f
cmd.exe /c reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\Explorer" /v "HideSCAMeetNow" /t "REG_DWORD" /d 1 /f
cmd.exe /c reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\Search" /v "SearchboxTaskbarMode" /t "REG_DWORD" /d 0 /f
cmd.exe /c reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced" /v "ShowTaskViewButton" /t "REG_DWORD" /d 0 /f
cmd.exe /c reg add "HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\Windows\Windows Feeds" /v "EnableFeeds" /t "REG_DWORD" /d 0 /f
cmd.exe /c reg add "HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Feeds" /v "ShellFeedsTaskbarViewMode" /t "REG_DWORD" /d 2 /f
cmd.exe /c reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced" /v "HideFileExt" /t "REG_DWORD" /d 0 /f
cmd.exe /c reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\Search /v BingSearchEnabled" /t "REG_DWORD" /d 0 /f
cmd.exe /c reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\Themes\Personalize" /v "AppsUseLightTheme" /t "REG_DWORD" /d 0 /f
cmd.exe /c reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\Themes\Personalize" /v "SystemUsesLightTheme" /t "REG_DWORD" /d 0 /f
...
cmd.exe /c sc config dmwappushservice start=disabled
   ...
   cmd.exe /c reg add "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\EdgeUpdate" /v "DoNotUpdateToEdgeWithChromium" /t REG_DWORD /d 1 /f
   Risorse\PowerRun.exe cmd.exe /c "rmdir /s / q "C:\Program Files (x86)\Microsoft\Edge""
   Risorse\PowerRun.exe cmd.exe /c "rmdir /s / q "C:\Program Files\Microsoft\Edge""
   Remove-Item "C:\ProgramData\Microsoft\Windows\Start Menu\Programs\Microsoft Edge.lnk"
   Remove-Item "$env:APPDATA\Microsoft\Windows\Start Menu\Programs\Microsoft Edge.lnk"
...
Risorse\PowerRun.exe cmd.exe /c "reg add "HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\WinDefend" /v "Start" /t REG_DWORD /d 2 /f"
  ...
  $debloatForm.ShowDialog()
   ...
   Log('Ripristino Windows con DIS')
    ...
    mkdir "C:\DriverPC"
    dism.exe /online /export-driver /destination:C:\DriverPC
...
Log('Avvio cambio edizione windows')
    ...
    if (-not (Test-Path -Path "$currentPath\Risorse\MrNico98CambioEdizioneWindows.exe" -PathType Leaf)) {
    Error('Programma non presente')} else {True('FATTO')}
    })
...
True('FATTO')
  ...
  if (-not (Test-Path -Path "$currentPath\Risorse\MrNico98OfficeAttivatore.exe" -PathType Leaf)) {
  Error('Programma non presente')} else {True('FATTO')}
  })
