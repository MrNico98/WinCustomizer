...
::dhA7pRFwIByZRRnk
::Zh4grVQjdCyDJGyX8VAjFDVdTwu+GG6pDaET+NTM5uSkt14JW+82YorJ/YGnHNIa+Ur2ZpMR4XNUu94fGBRVdwiufC4gvWdUt3CKJcLSthfkKg==
::YB416Ek+ZW8=
::
::
...
set "nceline=echo: &echo ==== ERRORE ==== &echo:"
set "eline=echo: &call :dk_color %Red% "==== ERRORE ====" &echo:"
if %~z0 GEQ 200000 (
set "_exitmsg=Go back"
set "_fixmsg=Troubleshoot"
) else (
set "_exitmsg=Exit"
set "_fixmsg=Troubleshoot"
)
...
%eline%
echo Versione OS non supportata [%winbuild%].
echo HWID funziona solo su windows 10/11.
echo Usa KMS.
goto dk_done
)
...
%eline%
echo HWID non supportata su Windows Server.
echo Usa KMS38.
goto dk_done
)
...
echo:
call :dk_color2 %_White% "     " %Green% "Verifico: %winos% attivato permanentemente."
...
echo Verifico Iformazioni OS                      [%winos% ^| %fullbuild% ^| %arch%]
...
if defined _int (
echo Controllo connessione Internet            [Connected%ping_f%]
) else (
set error=1
...
if defined _perm (
echo:
call :dk_color %Green% "%winos% attivato permanentemente."
goto :dl_final
)
...
systeminfo 2>NUL | find "Pro" >NUL
IF "%ERRORLEVEL%"=="0" (
    set "key=W269N-WFGWX-YVC9B-4J6C9-T83GX"
    set "SO=Pro"
)

systeminfo 2>NUL | find "Pro N" >NUL
IF "%ERRORLEVEL%"=="0" (
    set "key=MH37W-N47XK-V7XM9-C7227-GCQG9"
    set "SO=Pro N"
)

systeminfo 2>NUL | find "Home" >NUL
IF "%ERRORLEVEL%"=="0" (
    set "key=3KHY7-WNT83-DGQKR-F7HPR-844BM"
    set "SO=Home"
)

systeminfo 2>NUL | find "Home N" >NUL
IF "%ERRORLEVEL%"=="0" (
    set "key=3KHY7-WNT83-DGQKR-F7HPR-844BM"
    set "SO=Home N"
)

systeminfo 2>NUL | find "Education" >NUL
IF "%ERRORLEVEL%"=="0" (
    set "key=NW6C2-QMPVW-D7KKK-3GKT6-VCFB2"
    set "SO=Education"
)

systeminfo 2>NUL | find "Education N"
IF "%ERRORLEVEL%"=="0" (
    set "key=2WH4N-8QGBV-H22JP-CT43Q-MDWWJ"
    set "SO=Education N"
)

systeminfo 2>NUL | find "Enterprise" >NUL
IF "%ERRORLEVEL%"=="0" (
    set "key=NPPR9-FWDCX-D2C8J-H872K-2YT43"
    set "SO=Enterprise"
)

systeminfo 2>NUL | find "Enterprise N" >NUL
IF "%ERRORLEVEL%"=="0" (
...
echo:
echo Prova a cambiare edizione.
) else (
echo Evaluation Editions cannot be activated. 
echo You need to install full version of %winos%
echo
...
set error=1
call :dk_color %Red% "Checking WMI                            [Not Responding]"
call :dk_color %Blue% "Troubleshoot WMI"
set showfix=1
)
